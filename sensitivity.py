...
def load_sensitive_terms(path: str) -> Dict[str, Optional[str]]:
    ...
    terms = {}
    try:
        log("DEBUG", f"Opening sensitivity terms file at: {path}", prefix="SENS")
        with open(path, 'r', encoding='utf-8') as f:
            for line_number, line in enumerate(f, start=1):
                original_line = line.strip()
                if not original_line or original_line.startswith("#"):
                    log("DEBUG", f"Skipping comment/empty line {line_number}", prefix="SENS")
                    continue
                if "=>" in original_line:
                    term, replacement = map(str.strip, original_line.split("=>", 1))
                    log("DEBUG", f"Parsed replacement line {line_number}: '{term}' => '{replacement}'", prefix="SENS")
                    terms[term.lower()] = replacement
                else:
                    log("DEBUG", f"Parsed flag-only line {line_number}: '{original_line}'", prefix="SENS")
                    terms[original_line.lower()] = None
        log("INFO", f"Loaded {len(terms)} sensitive terms", prefix="SENS")
    except Exception as e:
        log("ERROR", "Failed to load sensitive terms file", prefix="SENS", exception=e)
        raise
    return terms
...
def scan_for_sensitive_terms(text: str, terms: Dict[str, Optional[str]]) -> List[Tuple[str, Optional[str]]]:
    ...
    found = []
    lowered = text.lower()
    log("DEBUG", f"Scanning text of length {len(text)} for {len(terms)} terms", prefix="SENS")

    for term, replacement in terms.items():
        if term in lowered:
            log("DEBUG", f"Match found: '{term}' → Suggested: '{replacement}'", prefix="SENS")
...
def check_finding_for_sensitivities(finding: Finding, terms: Dict[str, Optional[str]]) -> Dict[str, List[Tuple[str, Optional[str]]]]:
    ...
    log("DEBUG", f"Checking finding ID: {finding.id} for sensitive content", prefix="SENS")
    for field in fields_to_check:
        content = getattr(finding, field, None)
        if not content or not isinstance(content, str):
            log("DEBUG", f"Skipping field '{field}' (empty or non-string)", prefix="SENS")
            continue
        matches = scan_for_sensitive_terms(content, terms)
        if matches:
            log("INFO", f"Sensitive terms found in '{field}': {matches}", prefix="SENS")
