...
def score_finding_similarity(f1: Finding, f2: Finding) -> float:
    ...
    """
    log("DEBUG", f"Scoring similarity between Finding A (ID: {f1.id}) and Finding B (ID: {f2.id})", prefix="MATCH")

    # Title similarity using token sort ratio (handles reordering well)
    title_score = fuzz.token_sort_ratio(f1.title or "", f2.title or "")
    log("DEBUG", f"Title score between '{f1.title}' and '{f2.title}': {title_score}", prefix="MATCH")
    ...
    if f1.description and f2.description:
        ...
        log("DEBUG", f"Description score between Finding A and B: {desc_score}", prefix="MATCH")
    else:
        log("DEBUG", "At least one finding is missing a description. Skipping desc score.", prefix="MATCH")
    ...
    if f1.finding_type and f2.finding_type:
        log("DEBUG", f"Finding types: A='{f1.finding_type}' B='{f2.finding_type}' → Type score: {type_score}", prefix="MATCH")
    else:
        log("DEBUG", "At least one finding is missing a finding_type. Skipping type score.", prefix="MATCH")
    ...
    log("DEBUG", f"[Weights: title={title_weight}, desc={desc_weight}, type={type_weight}] → Final score: {combined_score:.2f}", prefix="MATCH")  # Logs the breakdown of the computed score with applied weights
...
def fuzzy_match_findings(
    ...
    """
    log("INFO", f"Beginning fuzzy match on {len(list_a)} findings from A and {len(list_b)} from B", prefix="MATCH")
    ...
    for idx_a, finding_a in enumerate(list_a):
        log("DEBUG", f"Searching match for A#{idx_a} (ID: {finding_a.id})", prefix="MATCH")
        best_match = None
        best_score = 0
        best_idx_b = -1
        ...
        for idx_b, finding_b in enumerate(list_b):
            if idx_b in matched_indices_b:
                log("DEBUG", f"Skipping B#{idx_b} (already matched)", prefix="MATCH")
                continue

            score = score_finding_similarity(finding_a, finding_b)
            log("DEBUG", f"→ Score A#{idx_a} ↔ B#{idx_b}: {score:.2f}", prefix="MATCH")
        ...
        if best_score >= threshold and best_match:
            ...
            log("INFO", f"Matched A#{idx_a} (ID: {finding_a.id}) with B#{best_idx_b} (ID: {best_match.id}) at {best_score:.2f}", prefix="MATCH")
        else:
            unmatched_a.append(finding_a)
            log("DEBUG", f"No match found for A#{idx_a} (best was {best_score:.2f})", prefix="MATCH")

    unmatched_b = [b for idx, b in enumerate(list_b) if idx not in matched_indices_b]

    log("INFO", f"Fuzzy matched {len(matches)} pairs", prefix="MATCH")
    log("INFO", f"Unmatched: {len(unmatched_a)} in A, {len(unmatched_b)} in B", prefix="MATCH")
